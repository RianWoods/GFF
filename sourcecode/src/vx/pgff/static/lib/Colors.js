...
function makeSelectionColorPalette(w, h){
//    'idwinmodalbody'
    
    gelem("idwinmodal").style.display = "block"; 
    gelem("idwinmodaltitle").innerHTML = "Change colors"; 
    ...
    txtcol += "</tbody>"
    txtcol += "</table>";
    gelem("idwinmodalbody").innerHTML = txtcol;
    
    for(var i in CCTT.COLORST){
        idt = `tcoloridx`+i+``;
        CCTT.id = i;
        c = CCTT.interpolate();
        drawcolorpalette("#"+idt, w, h, c);
    }

}
function drawcolorpalette(idview, w, h, cf){
    var n = 100;
